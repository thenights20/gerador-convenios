...
const definirDuracao=$("definirDuracao"),duracaoField=$("duracaoField"),duracaoAnos=$("duracaoAnos");
function updateDurationControl(){
  const active=definirDuracao.checked;
  duracaoField.hidden=!active;
  duracaoAnos.required=active;
  definirDuracao.closest("label").querySelector("small").textContent=active?"Prazo determinado":"Desativado: prazo indeterminado";
  if(!active){duracaoAnos.value="";duracaoAnos.classList.remove("invalid")}
}
definirDuracao.addEventListener("change",updateDurationControl);
duracaoAnos.addEventListener("input",()=>duracaoAnos.classList.remove("invalid"));
...
function numeroPorExtenso(n){const nomes={1:"um",2:"dois",3:"três",4:"quatro",5:"cinco",6:"seis",7:"sete",8:"oito",9:"nove",10:"dez"};return nomes[n]||String(n)}
...
function updateCopyText(){
  $("copyDataText").value=buildCopyText();
}
function updateProgress(){
  ...
  $("generateButton").disabled=total!==15;
  updateCopyText();
}
...
async function generate(){
  ...
  const anos=Number(duracaoAnos.value),invalidDuration=definirDuracao.checked&&(!Number.isInteger(anos)||anos<1||anos>10);
  duracaoAnos.classList.toggle("invalid",invalidDuration);if(invalidDuration)valid=false;
  if(!valid){$("finalStatus").className="status error";$("finalStatus").textContent=invalidCpf?"CPF inválido. Confira os números informados.":invalidDuration?"Informe uma duração inteira entre 1 e 10 anos.":"Revise os campos destacados.";document.querySelector(".invalid")?.focus();return}
  ...
  try{
    ...
    if(definirDuracao.checked){
      const unidade=anos===1?"ano":"anos",contagem=anos===1?"contado":"contados";
      const clausulaVigencia=[
        {text:"CLÁUSULA 8ª - DA VIGÊNCIA:",bold:true},
        {text:`O presente termo de convênio para concessão de estágio obrigatório terá vigência por ${anos} (${numeroPorExtenso(anos)}) ${unidade}, ${contagem} a partir da data de sua assinatura, porém, havendo interesse em sua rescisão, a parte interessada notificará a parte contrária, por escrito, com antecedência de 30 (trinta) dias.`}
      ];
      const page4=pages[3];
      page4.drawRectangle({x:70,y:508,width:454,height:86,color:white});
      drawRichJustified(page4,clausulaVigencia,{x:76,y:580,width:442,regular,bold,size:12,lineHeight:20.7,maxLines:4});
    }
...
$("copyDataButton").addEventListener("click",async()=>{
  const text=$("copyDataText").value;
  try{
    await navigator.clipboard.writeText(text);
    $("copyStatus").className="copy-status success";
    $("copyStatus").textContent="Dados copiados.";
  }catch{
    $("copyDataText").select();
    const copied=document.execCommand("copy");
    $("copyStatus").className=`copy-status ${copied?"success":"error"}`;
    $("copyStatus").textContent=copied?"Dados copiados.":"Não foi possível copiar automaticamente.";
    window.getSelection()?.removeAllRanges();
  }
  setTimeout(()=>{$("copyStatus").textContent=""},2000);
});
$("clearButton").addEventListener("click",()=>{if(confirm("Deseja limpar todos os dados?")){$("form").reset();$("data").value=new Date().toISOString().slice(0,10);$("importStatus").textContent="";$("finalStatus").textContent="";$("copyStatus").textContent="";updateDurationControl();setTermType(termType,{clearIdentifier:false});updateProgress()}});
